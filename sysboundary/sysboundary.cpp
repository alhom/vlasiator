...
void SysBoundary::addParameters() {
   Readparameters::addComposing("boundaries.boundary", "List of boundary condition (BC) types to be used. Each boundary condition to be used has to be on a new line boundary = YYY. Available (20140113) are Outflow Static Ionosphere StaticIonosphere Conductingsphere, Maxwellian, User.");
   ...
   for (auto cellId : local_cells) {
      ...
      if(cell) {
         if (cell->sysBoundaryFlag == sysboundarytype::IONOSPHERE ||
             cell->sysBoundaryFlag == sysboundarytype::STATICIONOSPHERE || cell->sysBoundaryFlag == sysboundarytype::CONDUCTINGSPHERE) {
