...
void SysBoundary::addParameters() {
   Readparameters::addComposing(
       ...
       "boundary = YYY. Available options are: Outflow, Ionosphere, StaticIonosphere, Static, Conductingsphere, Maxwellian, User.");
...
bool SysBoundary::initSysBoundaries(Project& project, creal& t) {
   ...
   for (it = sysBoundaryCondList.begin(); it != sysBoundaryCondList.end(); it++) {
      if (*it == "Outflow") {
         ...
         if((faces[0] || faces[1]) && P::xcells_ini < 2) {
         ...
         if((faces[2] || faces[3]) && P::ycells_ini < 2) {
         ...
         if((faces[4] || faces[5]) && P::zcells_ini < 2) {
   ...
   for (auto cellId : local_cells) {
      ...
      if (cell) {
         if (cell->sysBoundaryFlag == sysboundarytype::IONOSPHERE || 
             cell->sysBoundaryFlag == sysboundarytype::CONDUCTINGSPHERE ||
