...
void computeSpatialSourceCellsForPencil(const dccrg::Dccrg<SpatialCell,dccrg::Cartesian_Geometry>& mpiGrid,
   ...
   for(int i = VLASOV_STENCIL_WIDTH - 1; i >= 0 ;--i){
      if(mpiGrid[sourceCells[i]] == NULL || mpiGrid[sourceCells[i]]->parameters[CellParams::CELLID] == sourceCells[i] ||  mpiGrid[sourceCells[i]]->sysBoundaryFlag == sysboundarytype::DO_NOT_COMPUTE)
