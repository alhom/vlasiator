...
//Is cell translated? It is not translated if DO_NOT_COMPUTE or if it is sysboundary cell and not in first sysboundarylayer
bool do_translate_cell(const SpatialCell* const SC){
   if(!SC){
      ...
      std::stringstream ss;
      ss << myRank << ": Null SpatialCell in do_translate_cell ";
...
bool check_is_active(const dccrg::Dccrg<SpatialCell,dccrg::Cartesian_Geometry>& mpiGrid,
   ...
   if (P::vlasovSolverGhostTranslate) { // or timeclasses
...
void findNeighborhoodCells(const dccrg::Dccrg<SpatialCell,dccrg::Cartesian_Geometry>& mpiGrid,
   ...
   if (timeclass == -1){ // No need to fool around with timeclasses
      ...
      for (const auto& nbrPair : *NbrPairs) {
         ...
         if (ncell->has_timeclass(timeclass)) {
            foundCells.push_back(nbrPair.first);
         }
      }
   }
   ...
   if (localPropagatedCells.size() == 0) {
      ...
      */
      ...
      for (const CellID c : localPropagatedCells) {
         ...
         findNeighborhoodCells(mpiGrid, c, dimension, activeSearchLength, foundCells, tc);
         for (const CellID cid: foundCells) {
            if(tc == -1 || mpiGrid[cid]->has_active_timeclass(tc)){
               activey.insert(cid);
            }
         }
...
CellID selectPositiveNeighbor(const dccrg::Dccrg<SpatialCell,dccrg::Cartesian_Geometry> &grid,
                      const CellID id, const int dimension = 0, const uint path = 0, const int timeclass = -1, const uint tictoc = 0) {
   ...
   if (P::currentMaxTimeclass == 0 || timeclass == -1) {
      ...
      std::map<uint,std::map<uint,std::unordered_set<CellID>>>* active_list;
      if (tictoc == Timeclasses::TIC) {
         active_list = &timeghost_active_tic;
      }
      else if (tictoc == Timeclasses::TOC) {
         active_list = &timeghost_active_toc;
      }
      if (check_is_active(grid, myNeighbors[neighborIndex], dimension, (*active_list)[timeclass])) {
      neighbor = myNeighbors[neighborIndex];
      } else {
         #ifdef DEBUG_PENCILS
            std::cerr << __FILE__ <<":"<<__LINE__ << " " << myNeighbors[neighborIndex] << " not active "<< std::endl;
...
void buildPencilsWithNeighbors( const dccrg::Dccrg<SpatialCell,dccrg::Cartesian_Geometry> &grid,
					setOfPencils &pencils, const pair<int,CellID> seedId,
					vector<CellID> ids, const uint dimension,
					vector<uint> path, const vector<pair<int,CellID>> &endIds, const uint tictoc = 0) {
   ...
   while (id != INVALID_CELLID) {
      ...
      for (int tmpPath = 0; tmpPath < 4; ++tmpPath) {
         ...
         nextNeighbor = selectPositiveNeighbor(grid,id,dimension,tmpPath,timeclass, tictoc);
         #ifdef DEBUG_PENCILS
            std::cerr << __FILE__ <<":"<<__LINE__ << " " << myRank << ": try " << nextNeighbor <<std::endl;
         #endif
      ...
      if (refLvl > 0) {
         ...
         if ( static_cast<int>(path.size()) >= refLvl ) {
            ...
            nextNeighbor = selectPositiveNeighbor(grid,id,dimension,path[refLvl - 1],timeclass, tictoc);
            if (nextNeighbor != INVALID_CELLID) {
            ...
            for ( uint newPath : {0,1,2,3} ) {
               ...
               nextNeighbor = selectPositiveNeighbor(grid,id,dimension,newPath,timeclass, tictoc);
               ...
               if ( newPath == 3 ) {
                  ...
                  buildPencilsWithNeighbors(grid,pencils,{timeclass,id},ids,dimension,myPath,endIds, tictoc);
               }
            }
         }
      } // Closes if (refLvl == 0)
      ...
      if (nextNeighbor != INVALID_CELLID) {
         ...
         #ifdef DEBUG_PENCILS
            std::cerr << __FILE__<<":"<<__LINE__ << " " << myRank << ": Next neighbor is " << nextNeighbor << ". Inseeds: "<< inseeds << ", do_translate: " << do_translate_cell(grid[nextNeighbor]) << std::endl;
         #endif
...
int getNeigborhoodStencilLength(){
   ...
   if (P::vlasovSolverGhostTranslate){
      if (P::initialMaxTimeclass == 0){
         ...
         stencil=max(P::timeclassExactHaloExtent, (int)P::vlasovSolverGhostTranslateExtent);//+P::timeclassOuterHaloExtent;
      }
   }
   else{
...
// #endif
   ...
   for (uint i=0; i<propagatedCells.size(); i++) {
      ...
      #ifdef DEBUG_PENCILS
         std:cerr << myRank << " Checking for seed: " << celli << "\n";
      #endif
...
//#pragma omp critical // TODO commented pragmatically, figure out
   ...
   // std::map<uint,std::unordered_set<CellID>> ghostTranslate_active(ghostTrans)
      ...
      #ifdef DEBUG_PENCILS
         std::cerr << myRank << " Checking for seed: " << celli << ", phase A\n";
      #endif
      for (const auto& [neighbor, dir] : mpiGrid.get_face_neighbors_of(celli) ) {
         if ( dir == -((int)dimension + 1) ) {
            ...
            // std::cout << "active " << check_is_active(mpiGrid, neighbor, dimension, timeghost_active_tic[timeclass], getLocalCells())<<"\n";
            if (P::currentMaxTimeclass == 0){
               ...
               #endif
               if ( (myIndices[dimension] < nbrIndices[dimension]) ||
                  !check_is_active(mpiGrid, neighbor, dimension, active_list->at(timeclass)) ||
                  !do_translate_cell(mpiGrid[neighbor]))
               {
                  addToSeedIds = true;
                  break;
...
//#pragma omp critical (pencil_seedIds_push_back) // TODO commented pragmatically, figure out
      ...
      #ifdef DEBUG_PENCILS
         std::cerr << myRank << " Checking for seed: " << celli << ", phase B\n";
      #endif
...
//#pragma omp critical (pencil_seedIds_push_back) // TODO commented pragmatically, figure out
   ...
   #ifdef DEBUG_PENCILS
      std::cerr << "Rank " << myRank << ", dimension " << dimension << ", Seed ids are: ";
      for (const auto& seedId : seedIds) {
         std::cerr << seedId.first << ": " << seedId.second << "; ";
      }
      std::cerr << endl;
   #endif
}
   ...
   for (uint pencili = 0; pencili < pencils.N; ++pencili) {
      ...
      */
      ...
      for (auto it = distances.begin(); it != distances.end(); ++it) {
         ...
         if (P::initialMaxTimeclass > 0){
            if (tictoc == Timeclasses::TIC) {
               if (foundcells >= P::timeclassOuterHaloExtent) {
                  break; // checked enough distances
               }   
            }
            else{ // Timeclasses::TOC
               if (foundcells >= P::timeclassExactHaloExtent) {
                  break; // checked enough distances
               }
            }
      ...
      if(P::vlasovSolverGhostTranslate){
         if (P::currentMaxTimeclass > 0) {
            ...
            for (int timeclass = 0; timeclass <= P::currentMaxTimeclass; ++timeclass){
               ...
               // std::cout << "\n";
               getSeedIds(mpiGrid, tc_propagatedCells[timeclass], dimension, seedIds, timeclass, tictoc);
   
               maxt = timeclass;
            }
         }
         else{
            getSeedIds(mpiGrid, propagatedCells, dimension, seedIds, 0, 0);
         }
      }
      else{
         getSeedIds(mpiGrid, propagatedCells, dimension, seedIds, 0, 0);
   ...
   #pragma omp parallel
      {
         ...
         for (uint i=0; i<seedIds.size(); i++) {
            ...
            buildPencilsWithNeighbors(mpiGrid, thread_pencils, seedIds[i], ids, dimension, path, seedIds, tictoc);
         }
         // if(exit) throw 123;
      ...
      check_ghost_cells(mpiGrid,(*pencilSet)[dimension],dimension, tictoc);
